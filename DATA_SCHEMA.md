...
```
purchase_orders (PO_No, Item_ID, Supplier_ID, Expected_Date)
    ↓ (PO_No)
...
#### 5. **Inventory**
```
inventory (Item_ID, Location, Qty_On_Hand)
    ↓ (Item_ID)
...
- `Location`: Warehouse/stock location
...
#### purchase_orders.csv
- `Expected_Date`: When the PO was expected to arrive

#### goods_receipts.csv
- `Receipt_Date`: Actual date when goods were received
- **Delay Calculation**: `Receipt_Date - Expected_Date`
