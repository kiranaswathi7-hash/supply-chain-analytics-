...
def show():
    ...
    location_stock = data['inventory'].groupby('Location')['Qty_On_Hand'].sum().reset_index()
    ...
    fig_location.update_layout(xaxis_title="Location", yaxis_title="Quantity on Hand")
    ...
    if len(critical_items) > 0:
        ...
        with col_filter1:
            filter_location = st.selectbox(
                'Filter by Location',
                ['All'] + list(critical_items['Location'].unique())
            )
        ...
        if filter_location != 'All':
            display_items = display_items[display_items['Location'] == filter_location]
        if filter_risk != 'All':
        ...
        st.dataframe(
            ...
            column_config={
                ...
                'Risk_Category': st.column_config.SelectColumnColumn(
