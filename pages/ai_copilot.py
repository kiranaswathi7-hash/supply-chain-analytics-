...
def show():
    ...
    st.markdown(f"""
    <div style="padding: 1rem; background-color: {theme['card_bg']}; border-radius: 0.5rem; margin-bottom: 1rem; color: {theme['text_color']};">
        <strong style="color: {theme['header_color']};">Ask me questions about your supply chain data:</strong>
        <ul style="margin-top: 0.5rem; color: {theme['text_color']};">
...
def get_items_below_reorder_point(data):
    ...
    response += "| Item ID | Name | Type | Warehouse | On Hand | Reorder Point | Shortage |\n"
    response += "|---------|-------------|------|----------|---------|---------------|----------|\n"
    
    for _, row in critical_items.head(20).iterrows():
        response += f"| {row['Item_ID']} | {row['Item_Name']} | {row['Item_Type']} | {row['Warehouse']} | {row['Qty_On_Hand']} | {row['Reorder_Point']} | {row['Shortage']} |\n"
